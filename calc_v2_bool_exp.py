...
def t_VNAME(t):
    r'[a-zA-Z_][a-zA-Z_0-9]*-IF-THEN-END-ELSE'
    t.type = 'VNAME'
    return t
...
def p_calc_v2_bool_exp(p):
    '''
    calc_q3 : expression_in
        | var_assign
        | ifthen
        | ifthenelse
        | empty
...
def p_ifthen(p):
    '''
    ifthen : IF expression_in THEN var_assign END
        | IF expression_in THEN expression_in END
    '''
    if run(p[2]):
        print(p[4])
        p[0] = run(p[4])
...
def p_ifthenelse(p):
    ...
    ifthenelse : IF expression_in THEN expression_in ELSE expression_in END
        | IF expression_in THEN var_assign ELSE expression_in END
        | IF expression_in THEN expression_in ELSE var_assign END
        | IF expression_in THEN var_assign ELSE var_assign END
    '''
    if run(p[2]):
        p[0] = run(p[4])
    else:
...
def p_var_assign(p):
    '''
    var_assign : VNAME EQULS expression_in
    '''
    p[0] = ('=', p[1], p[3])
